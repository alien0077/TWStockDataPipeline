...
def materialize_contract_baseline(repo: str, sha: str, root: Path, target_date: date) -> None:
    sys.path.insert(0, str(ROOT / "src"))
    from twstock_pipeline.github_api import GitHubGitDataAPI

    paths = list(CONTRACTS.values())
    paths.append(f"data/meta/actions/{target_date.year}.json")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="twstock-e2e-") as tmp:
        ...
        materialize_contract_baseline(args.repo, args.baseline_sha, baseline, args.date or date.today())
